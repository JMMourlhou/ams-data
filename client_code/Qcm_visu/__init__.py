...
class Qcm_visu(Qcm_visuTemplate):
    def __init__(self, pdf_mode=False, **properties):
        ...
        for row in rows :
            ...
            self.cp = ColumnPanel(role="default",
            ...
            self.xy_panel.add_component(self.cp, x=xx, y=yy, width=self.browser_width-10)   # changt jm   (-10)
            """
           # Création du column panel de la question et check boxes
            #self.cp = ColumnPanel(role="outlined-card",
            ...
            self.cp.tag.nom = "column"    #en fait 
            self.xy_panel.add_component(self.cp, x=xx, y=yy, width=self.browser_width-10)   # changt jm   (-10) 
            ...
            self.quest = TextArea(text=question,
                        ...
                        spacing_above=None,
                        spacing_below=None,
                        height=110           #hauteur fixe (contient au moins 4 lignes sur tel) pour faciliter l'espacement entre question et l'affichage des boutons valider /retour
                        )
