...
class ItemTemplate25(ItemTemplate25Template):
    ...
    def button_annuler_click(self, **event_args):
        ...
        r = alert(
            "Voulez-vous vraiment effacer ce lieu ?",
            dismissible=False,
            buttons=[("oui", True), ("non", False)],
        )
        if r:  # oui
            result, nb, liste = anvil.server.call(
                "del_lieu", self.item, self.item["lieu"]
            )
            if result is not True:
                ...
                alert(
                    f"Effacement non effectué, ce lieu est utilisé dans {nb} stage(s) :\nStage(s): {detail}"
                )
                return
    ...
    def button_modif_click(self, **event_args):
        """This method is called when the button is clicked"""
        r = alert(
            "Voulez-vous vraiment modifier ce Lieu ?",
            buttons=[("oui", True), ("non", False)],
        )
        if r:  # oui
