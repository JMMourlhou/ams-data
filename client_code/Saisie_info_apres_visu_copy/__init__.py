from ._anvil_designer import Saisie_info_apres_visu_copyTemplate
from anvil import *
...
class Saisie_info_apres_visu_copy(Saisie_info_apres_visu_copyTemplate):
    def __init__(self, mel, num_stage=0, intitule="", provenance="", **properties):
        ...
        self.drop_down_fi.items = [
            (r["intitule_fi"], r) for r in app_tables.mode_financement.search()
        ]
        ...
        if user:
            ...
            self.text_box_role.text = user["role"]
            if user["role"] == "A":
