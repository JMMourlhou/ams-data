...
class Mail_subject_attach_txt(Mail_subject_attach_txtTemplate):
    def __init__(
        self, stagiaire_inscrit_row, **properties
        ...
        if screen_size < 800:
            pr = stagiaire_inscrit_row["prenom"][
                0
            ].capitalize()  # 1ere lettre du prénom
            self.label_3.text = (
                "Pré-R perso pour "
                + nom
                + " "
                + pr
                + "  / "
                + stagiaire_inscrit_row["stage_txt"]
            )
        else:
            pr = stagiaire_inscrit_row["prenom"].capitalize()
            self.label_3.text = (
                "Pré-Requis personnalisés pour "
                + nom
                + " "
                + pr
                + "  / "
        ...
        liste_pr_stagiaire = app_tables.pre_requis_stagiaire.search(
            q.fetch_only(
                "requis_txt",
