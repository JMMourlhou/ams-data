...
class ItemTemplate3_copy(ItemTemplate3_copyTemplate):
    ...
    def file_loader_1_change(self, file, **event_args):
        if file is not None:  # pas d'annulation en ouvrant choix de fichier
            ...
            new_file_name = Pre_R_doc_name.doc_name_creation(
                self.stage_num, self.item_requis, self.email
            )  # extension non incluse
            print("new file name: ", new_file_name)

            # Type de fichier ?
            path_parent, file_name, file_extension = anvil.server.call(
                "path_info", str(file.name)
            )
            ...
            if (
                ...
                or file_extension == ".jif"
                or file_extension == ".png"
            ):
                # Sauvegarde du 'file' jpg
                result, thumb = anvil.server.call(
                    "modify_pre_r_par_stagiaire", self.item, file, new_file_name, ".jpg"
                )
            ...
            if file_extension == ".pdf":
                ...
                liste_images = anvil.server.call(
                    "pdf_into_jpg",
                    self.stage_num,
                    self.item_requis,
                    self.email,
    ...
    def button_del_click(self, **event_args):
        """This method is called when the button is clicked"""
        result = anvil.server.call(
            "pr_stagiaire_del", self.email, self.stage_num, self.item_requis
        )
        if result:
